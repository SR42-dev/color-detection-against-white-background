...
img = imageFrame[73:203, 417:549]
# cv2.imshow("temp", img)

hsvFrame = cv2.cvtColor(imageFrame, cv2.COLOR_BGR2HSV)
whiteHSV = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)
...
white_mask = cv2.inRange(whiteHSV, white_lower, white_upper)
...
res_white = cv2.bitwise_and(img, imageFrame,mask=white_mask)
...
for pic, contour in enumerate(contours):
    ...
    if (area > 1000) and (area < 2000):
        x, y, w, h = cv2.boundingRect(contour)
        imageFrame = cv2.rectangle(imageFrame, (x, y), (x + w, y + h), (0, 255, 0), 2)
        cv2.putText(imageFrame, "White", (x + 25, y + 25), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255))
